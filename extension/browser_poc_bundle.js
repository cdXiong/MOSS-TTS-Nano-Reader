...
// src/browser_poc_main.js
var DEFAULT_BROWSER_MODEL_PATH = "models";
var EXTERNAL_BROWSER_MODEL_KEY = "browser-onnx-external";
var DEFAULT_BACKEND_CONFIG = {
  backend: "browser_onnx",
  browserModelPath: DEFAULT_BROWSER_MODEL_PATH
};
var SAMPLE_MODE_GREEDY2 = "greedy";
var SAMPLE_MODE_FIXED2 = "fixed";
var SAMPLE_MODE_FULL2 = "full";
var DEFAULT_TTS_SETTINGS = {
  sampleMode: SAMPLE_MODE_FIXED2,
  doSample: true,
...
var ui = {
  usePathButton: document.getElementById("btn-use-model-path"),
  loadButton: document.getElementById("btn-load-poc-assets"),
  reloadVoicesButton: document.getElementById("btn-reload-voices"),
  saveVoiceButton: document.getElementById("btn-save-uploaded-voice"),
  modelDirPathInput: document.getElementById("model-dir-path"),
  threadCountInput: document.getElementById("cpu-thread-count"),
  modelDirValue: document.getElementById("model-dir-value"),
  enableWeTextProcessingToggle: document.getElementById("wetext-processing-enabled"),
  enableNormalizeTtsTextToggle: document.getElementById("normalize-tts-text-enabled"),
  initialPlaybackDelayInput: document.getElementById("initial-playback-delay"),
  attnImplementationSelect: document.getElementById("attn-implementation"),
  voiceCloneMaxTextTokensInput: document.getElementById("voice-clone-max-text-tokens"),
  ttsMaxBatchSizeInput: document.getElementById("tts-max-batch-size"),
  codecMaxBatchSizeInput: document.getElementById("codec-max-batch-size"),
  voiceSelect: document.getElementById("voice-select"),
  sampleSelect: document.getElementById("sample-select"),
  textInput: document.getElementById("tts-text"),
  sampleModeSelect: document.getElementById("sample-mode"),
  uploadedVoiceNameInput: document.getElementById("uploaded-voice-name"),
  uploadedVoiceGroupInput: document.getElementById("uploaded-voice-group"),
  uploadedVoiceFileInput: document.getElementById("uploaded-voice-file"),
  synthesizeButton: document.getElementById("btn-run-non-streaming"),
...
var AudioQueuePlayer = class {
  constructor() {
    this.audioContext = null;
    this.nextPlaybackTime = 0;
    this.initialPlaybackDelaySeconds = DEFAULT_TTS_SETTINGS.initialPlaybackDelaySeconds;
  }
  configure({ initialPlaybackDelaySeconds = DEFAULT_TTS_SETTINGS.initialPlaybackDelaySeconds } = {}) {
    const nextDelay = Math.max(0, Number(initialPlaybackDelaySeconds));
    this.initialPlaybackDelaySeconds = Number.isFinite(nextDelay) ? nextDelay : DEFAULT_TTS_SETTINGS.initialPlaybackDelaySeconds;
  }
  async ensureContext() {
  ...
  async queueFloatChunk({ channels, sampleRate, chunkData }) {
    ...
    const startAt = Math.max(
      this.nextPlaybackTime || context.currentTime + this.initialPlaybackDelaySeconds,
...
function normalizeSampleMode2(rawSampleMode, rawDoSample = true) {
  const normalized = String(rawSampleMode || "").trim();
  if ([SAMPLE_MODE_GREEDY2, SAMPLE_MODE_FIXED2, SAMPLE_MODE_FULL2].includes(normalized)) {
    return normalized;
  }
  if (normalized === "mixed3") {
    return rawDoSample === false ? SAMPLE_MODE_GREEDY2 : SAMPLE_MODE_FIXED2;
  }
  return rawDoSample === false ? SAMPLE_MODE_GREEDY2 : SAMPLE_MODE_FIXED2;
}
...
function slugifyVoiceIdBase(displayName) {
  const asciiText = String(displayName).normalize("NFKD").replace(/[^\x00-\x7F]/g, "");
...
function isResourceUrl2(pathValue) {
  return /^[A-Za-z][A-Za-z0-9+.-]*:\/\//.test(String(pathValue || "").trim());
}
function normalizePackagedModelPath(pathValue) {
  const parts = String(pathValue || "").trim().replace(/\\/g, "/").replace(/^\/+/, "").split("/").filter(Boolean);
  const resolvedParts = [];
...
function normalizeStoredBrowserModelPath(rawPath = "") {
  ...
  if (lowered === "models/browser_onnx_poc_dynamic_int8" || lowered.endsWith("/models/browser_onnx_poc_dynamic_int8")) {
    return "models";
  }
  if (lowered === "models/browser_onnx_poc" || lowered.endsWith("/models/browser_onnx_poc")) {
    return "models";
  }
  if (lowered === "models/moss-tts-nano-100m-onnx" || lowered === "models/moss-audio-tokenizer-nano-onnx") {
    return "models";
  }
  return normalizedPath;
}
function getBrowserModelStore2() {
...
function isDefaultPackagedBrowserModelPath(pathValue = "") {
  return normalizeStoredBrowserModelPath(pathValue) === DEFAULT_BROWSER_MODEL_PATH;
}
...
async function maybeProvisionExternalBrowserModel(pathValue) {
  const normalizedInput = normalizeMetadataText(pathValue);
  if (!normalizedInput) {
    throw new Error("Enter a local browser ONNX model root path first.");
  }
  const normalizedPath = normalizeStoredBrowserModelPath(normalizedInput);
  if (!isDefaultPackagedBrowserModelPath(normalizedPath)) {
    return {
      browserModelPath: normalizedPath,
      downloaded: false,
      external: false
    };
  }
  if (await packagedBrowserModelExists(normalizedPath)) {
    return {
      browserModelPath: normalizedPath,
      downloaded: false,
      external: false
  ...
  const store = getBrowserModelStore2();
  const originalLoadText = ui.loadButton.textContent;
  const progressHandler = (progress) => {
    ...
    if (progress?.phase === "download" && Number.isFinite(progress?.fileIndex) && Number.isFinite(progress?.fileCount)) {
      ui.loadButton.textContent = `Downloading ${progress.fileIndex}/${progress.fileCount}`;
    }
    setStatus(message);
    logLine(message);
  };
  ...
  try {
    ...
    return {
      ...
      external: true,
      label: result.label || EXTERNAL_BROWSER_MODEL_KEY
    };
  } finally {
    ui.loadButton.textContent = originalLoadText;
  }
}
function resolveBrowserModelPath(browserModelPath) {
  ...
  if (isResourceUrl2(normalizedPath)) {
...
function normalizeBackendConfig(rawConfig = {}) {
  const normalizedBackend = String(rawConfig?.backend || DEFAULT_BACKEND_CONFIG.backend).trim() === "server" ? "server" : "browser_onnx";
  const normalizedBrowserModelPath = String(
    normalizeStoredBrowserModelPath(rawConfig?.browserModelPath) || DEFAULT_BACKEND_CONFIG.browserModelPath || ""
  ).trim();
  return {
...
async function persistBackendConfig(config) {
...
async function loadBrowserUploadedVoices() {
  ...
  return Array.isArray(browserUploadedVoices) ? browserUploadedVoices.map(normalizeBrowserVoiceRow).filter(Boolean) : [];
}
...
function setPreparedTextPlaceholder(text = "No prepared text yet.", meta = "No prepared text yet.") {
  ...
  ui.preparedTextMeta.textContent = meta;
}
function updatePreparedTextDisplay(preparedText = {}) {
  const text = String(preparedText.text || "").trim();
  const method = String(preparedText.normalizationMethod || "none").trim() || "none";
  const warnings = Array.isArray(preparedText.warnings) ? preparedText.warnings.filter(Boolean) : [];
  const wetextRequested = Boolean(preparedText.wetextRequested);
  const wetextApplied = Boolean(preparedText.wetextApplied);
  const wetextAvailable = Boolean(preparedText.wetextAvailable);
  const metaParts = [
    `method=${method}`,
  ...
  ui.preparedTextMeta.textContent = metaParts.join(" | ");
  ui.preparedTextValue.textContent = text || "Prepared text is empty.";
}
function normalizeTtsSettings(rawSettings = {}) {
  const source = rawSettings || {};
  const normalized = { ...DEFAULT_TTS_SETTINGS, ...source };
  normalized.sampleMode = normalizeSampleMode2(
    source.sampleMode !== void 0 ? source.sampleMode : normalized.sampleMode,
    source.doSample !== void 0 ? source.doSample !== false : normalized.doSample !== false
  );
  normalized.doSample = normalized.sampleMode !== SAMPLE_MODE_GREEDY2;
  ...
  normalized.enableWeTextProcessing = normalized.enableWeTextProcessing !== void 0 ? normalized.enableWeTextProcessing !== false : legacyTextNormalizationEnabled !== false;
  normalized.enableNormalizeTtsText = normalized.enableNormalizeTtsText !== void 0 ? normalized.enableNormalizeTtsText !== false : legacyTextNormalizationEnabled !== false;
  ...
  normalized.attnImplementation = ["model_default", "sdpa", "eager"].includes(normalized.attnImplementation) ? normalized.attnImplementation : DEFAULT_TTS_SETTINGS.attnImplementation;
  normalized.voiceCloneMaxTextTokens = Math.max(
    25,
    parseInt(normalized.voiceCloneMaxTextTokens || DEFAULT_TTS_SETTINGS.voiceCloneMaxTextTokens, 10) || DEFAULT_TTS_SETTINGS.voiceCloneMaxTextTokens
  );
...
async function loadTtsSettings() {
  ...
  return normalizeTtsSettings(ttsSettings);
}
function getTtsSettingsFromUi() {
  const sampleMode = normalizeSampleMode2(ui.sampleModeSelect.value, true);
  return normalizeTtsSettings({
    sampleMode,
    doSample: sampleMode !== SAMPLE_MODE_GREEDY2,
    enableWeTextProcessing: ui.enableWeTextProcessingToggle.checked,
    enableNormalizeTtsText: ui.enableNormalizeTtsTextToggle.checked,
    initialPlaybackDelaySeconds: ui.initialPlaybackDelayInput.value,
    cpuThreads: ui.threadCountInput.value,
    attnImplementation: ui.attnImplementationSelect.value,
    voiceCloneMaxTextTokens: ui.voiceCloneMaxTextTokensInput.value,
    ttsMaxBatchSize: ui.ttsMaxBatchSizeInput.value,
    codecMaxBatchSize: ui.codecMaxBatchSizeInput.value
  });
}
function applyTtsSettingsToUi(settings) {
  const normalized = normalizeTtsSettings(settings);
  ui.sampleModeSelect.value = normalized.sampleMode;
  ui.enableWeTextProcessingToggle.checked = normalized.enableWeTextProcessing;
  ui.enableNormalizeTtsTextToggle.checked = normalized.enableNormalizeTtsText;
  ui.initialPlaybackDelayInput.value = normalized.initialPlaybackDelaySeconds.toFixed(2);
  ui.threadCountInput.value = String(normalized.cpuThreads);
  ui.attnImplementationSelect.value = normalized.attnImplementation;
  ui.voiceCloneMaxTextTokensInput.value = String(normalized.voiceCloneMaxTextTokens);
  ui.ttsMaxBatchSizeInput.value = String(normalized.ttsMaxBatchSize);
  ui.codecMaxBatchSizeInput.value = String(normalized.codecMaxBatchSize);
}
async function saveTtsSettings() {
  await chrome.storage.local.set({ ttsSettings: getTtsSettingsFromUi() });
}
...
function getPlayer() {
  if (!state.player) {
    state.player = new AudioQueuePlayer();
  }
  return state.player;
}
function currentThreadCount() {
  const settings = getTtsSettingsFromUi();
  return settings.cpuThreads > 0 ? settings.cpuThreads : Math.max(1, navigator.hardwareConcurrency || DEFAULT_TTS_SETTINGS.cpuThreads);
}
function syncControls() {
  const hasPath = Boolean(state.localPathRoot);
  const hasVoice = Boolean(ui.voiceSelect.value);
  const hasText = Boolean(normalizeMetadataText(ui.textInput.value));
  const canRun = hasPath && hasVoice && hasText;
  ui.loadButton.disabled = !hasPath;
  ui.reloadVoicesButton.disabled = !hasPath;
  ui.saveVoiceButton.disabled = !hasPath;
  ui.synthesizeButton.disabled = !canRun;
  ui.streamingButton.disabled = !canRun;
  ui.stopButton.disabled = false;
}
function renderSelectedModelPath() {
  if (!state.localPathRoot) {
    ui.modelDirValue.textContent = "No path selected.";
  } else if (state.localPathInput && state.localPathInput !== state.localPathRoot) {
    ui.modelDirValue.textContent = `${state.localPathInput}
=> ${state.localPathRoot}`;
  } else {
    ui.modelDirValue.textContent = state.localPathRoot;
  }
  syncControls();
}
function updateBrowserInfo() {
  ui.browserInfo.textContent = [
    `User agent: ${navigator.userAgent}`,
    `hardwareConcurrency=${navigator.hardwareConcurrency || "unknown"}`,
    `crossOriginIsolated=${String(crossOriginIsolated)}`
  ].join(" | ");
}
function renderVoiceOptions(voiceRows, preferredVoice = "") {
  const grouped = /* @__PURE__ */ new Map();
  for (const row of voiceRows) {
    const groupLabel = row.group || "Uploaded / Other";
  ...
  for (const [groupLabel, rows] of grouped.entries()) {
    const optgroup = document.createElement("optgroup");
    optgroup.label = groupLabel;
    for (const row of rows) {
      const option = document.createElement("option");
      option.value = row.voice;
      option.textContent = row.display_name || row.voice;
      if (row.voice === preferredVoice) {
        option.selected = true;
      }
      optgroup.appendChild(option);
    }
    ui.voiceSelect.appendChild(optgroup);
  }
  if (!ui.voiceSelect.value && voiceRows[0]) {
    ui.voiceSelect.value = preferredVoice && voiceRows.some((row) => row.voice === preferredVoice) ? preferredVoice : voiceRows[0].voice;
  }
}
function renderTextSamples(textSamples) {
  ui.sampleSelect.innerHTML = "";
  for (const sample of textSamples) {
    const option = document.createElement("option");
    option.value = sample.id;
    option.textContent = sample.label;
    option.dataset.sampleText = sample.text;
    ui.sampleSelect.appendChild(option);
  }
  if (textSamples[0] && !normalizeMetadataText(ui.textInput.value)) {
    ui.textInput.value = textSamples[0].text;
  }
}
async function refreshVoiceList(preferredVoice = "") {
  ...
  renderVoiceOptions([...builtinVoices, ...uploadedVoices], preferredVoice);
  syncControls();
}
async function ensureRuntime({ warmup = false, loadSynthesis = false } = {}) {
  if (!state.localPathRoot) {
    throw new Error("Enter a local browser ONNX model root path first.");
  }
  if (!state.runtime) {
    state.runtime = createBrowserOnnxTtsRuntime({
      logger: (message) => logLine(message)
    });
  }
  const threadCount = currentThreadCount();
  const runtimeKey = `${state.localPathRoot}::${threadCount}`;
  if (state.runtimeKey !== runtimeKey) {
    await state.runtime.configure({
      modelPath: state.localPathRoot,
      threadCount
    });
    state.runtimeKey = runtimeKey;
    state.prepared = false;
  }
  await state.runtime.ensureManifestLoaded();
  if (warmup) {
    await state.runtime.warmup();
    state.prepared = true;
  } else if (loadSynthesis) {
    await state.runtime.ensureSynthesisLoaded();
  }
  renderTextSamples(state.runtime.listTextSamples());
  return state.runtime;
}
async function applyModelPath(pathValue, { persist = true } = {}) {
  state.localPathInput = normalizeMetadataText(pathValue).replace(/[\\/]+$/, "");
  state.localPathRoot = resolveBrowserModelPath(state.localPathInput);
  renderSelectedModelPath();
  if (persist && state.localPathInput) {
    await persistBackendConfig({
      backend: "browser_onnx",
      browserModelPath: state.localPathInput
    });
  }
}
async function handleUsePath() {
  const rawPath = ui.modelDirPathInput.value;
  if (!normalizeMetadataText(rawPath)) {
    throw new Error("Enter a local browser ONNX model root path first.");
  }
  const provisionResult = await maybeProvisionExternalBrowserModel(rawPath);
  ui.modelDirPathInput.value = provisionResult.browserModelPath;
  await applyModelPath(provisionResult.browserModelPath, { persist: true });
  logLine(`Selected local model path: ${state.localPathRoot}`);
  if (provisionResult.external) {
    setStatus(
      provisionResult.downloaded
        ? `Downloaded browser_onnx models into browser-managed storage "${provisionResult.label}" and switched to it.`
        : `Using existing browser-managed browser_onnx models "${provisionResult.label}".`
    );
    return;
  }
  setStatus("Model path saved.");
}
async function handleLoadAndPrepare() {
  await handleUsePath();
  setStatus("Loading browser ONNX assets...");
  const runtime = await ensureRuntime({ warmup: true });
  await refreshVoiceList(ui.voiceSelect.value || runtime.listBuiltinVoices()[0]?.voice || "");
  setStatus("Browser ONNX models loaded and warmed up.");
}
async function handleReloadVoices() {
  await handleUsePath();
  await refreshVoiceList(ui.voiceSelect.value);
  setStatus("Voice list refreshed.");
}
function selectedSampleText() {
  return ui.sampleSelect.selectedOptions[0]?.dataset?.sampleText || "";
}
function fillTextFromSample() {
  const sampleText = selectedSampleText();
  if (sampleText) {
    ui.textInput.value = sampleText;
  }
  syncControls();
}
async function handleSaveUploadedVoice() {
  await handleUsePath();
  const displayName = normalizeUploadedDisplayName(ui.uploadedVoiceNameInput.value);
  const groupName = normalizeUploadedVoiceGroup(ui.uploadedVoiceGroupInput.value);
  const uploadedFile = ui.uploadedVoiceFileInput.files?.[0];
  if (!uploadedFile || uploadedFile.size <= 0) {
    throw new Error("Prompt audio file is required.");
  }
  setStatus("Encoding uploaded prompt audio...");
  const runtime = await ensureRuntime();
  await runtime.ensureCodecEncodeLoaded();
  const builtinVoices = runtime.listBuiltinVoices();
  const uploadedVoices = await loadBrowserUploadedVoices();
  const existingVoiceIds = [
    ...builtinVoices.map((row) => row.voice),
    ...uploadedVoices.map((row) => row.voice)
  ];
  const existingDisplayNames = new Set(
    [
      ...builtinVoices.map((row) => row.display_name || row.voice),
      ...uploadedVoices.map((row) => row.display_name || row.voice)
  ...
  const voiceId = buildUniqueVoiceId(displayName, existingVoiceIds);
  const newRow = {
    voice: voiceId,
  ...
  await refreshVoiceList(newRow.voice);
  ui.voiceSelect.value = newRow.voice;
  ui.uploadedVoiceNameInput.value = "";
  ui.uploadedVoiceFileInput.value = "";
  setStatus(`Uploaded voice saved: ${displayName}`);
}
async function stopPlayback() {
  state.currentRunToken += 1;
  await getPlayer().reset();
  setStatus("Playback stopped.");
}
async function runSynthesis(streaming) {
  await handleUsePath();
  const ttsSettings = getTtsSettingsFromUi();
  const text = normalizeMetadataText(ui.textInput.value);
  ...
  const runToken = state.currentRunToken + 1;
  state.currentRunToken = runToken;
  const player = getPlayer();
  player.configure({
    initialPlaybackDelaySeconds: ttsSettings.initialPlaybackDelaySeconds
  });
  await player.reset();
  const runtime = await ensureRuntime({ loadSynthesis: true });
  const uploadedVoices = await loadBrowserUploadedVoices();
  const selectedSample = runtime.listTextSamples().find((row) => row.id === ui.sampleSelect.value) || null;
  const preencodedTextTokenIds = selectedSample && text === selectedSample.text ? selectedSample.text_token_ids : null;
  let scheduledChunks = 0;
  setPreparedTextPlaceholder("Waiting for prepared text...", "Preparing browser_onnx text normalization...");
  setStatus(streaming ? "Running realtime streaming decode..." : "Running non-streaming synthesis...");
  logLine(`${streaming ? "Streaming" : "Non-streaming"} synthesis voice=${voiceName}`);
  await runtime.synthesizeVoiceClone({
    text,
    voiceName,
    preencodedTextTokenIds,
    extraVoices: uploadedVoices,
    sampleMode: ttsSettings.sampleMode,
    doSample: ttsSettings.doSample,
    streaming,
    enableNormalizeTtsText: ttsSettings.enableNormalizeTtsText,
    enableWeTextProcessing: ttsSettings.enableWeTextProcessing,
    voiceCloneMaxTextTokens: ttsSettings.voiceCloneMaxTextTokens,
    onPreparedText: async (preparedText) => {
      if (runToken !== state.currentRunToken) {
        return;
      }
      updatePreparedTextDisplay(preparedText);
    },
    isCancelled: () => runToken !== state.currentRunToken,
    onAudioChunk: async (chunk) => {
      if (runToken !== state.currentRunToken) {
        return;
      }
      const queued = await player.queueFloatChunk(chunk);
      if (queued) {
        scheduledChunks += 1;
      }
    }
  ...
  setStatus(
    streaming ? `Streaming playback queued. chunks=${scheduledChunks}` : `Non-streaming playback queued. chunks=${scheduledChunks}`
  );
}
async function bootstrap() {
  updateBrowserInfo();
  logLine("Browser ONNX PoC script initialized.");
  setPreparedTextPlaceholder();
  const [backendConfig, ttsSettings] = await Promise.all([
    loadBackendConfig(),
    loadTtsSettings()
  ]);
  applyTtsSettingsToUi(ttsSettings);
  const initialPath = backendConfig.browserModelPath || DEFAULT_BROWSER_MODEL_PATH;
  ui.modelDirPathInput.value = initialPath;
  await applyModelPath(initialPath, { persist: false });
  if (isDefaultPackagedBrowserModelPath(initialPath) && !await packagedBrowserModelExists(initialPath)) {
    logLine("Packaged browser_onnx models are missing. Load And Prepare will download them into browser-managed storage when needed.");
    setStatus("Packaged browser_onnx models missing. Load And Prepare will download them into browser-managed storage when needed.");
  }
  ui.usePathButton.addEventListener("click", () => void runAction(handleUsePath));
  ui.loadButton.addEventListener("click", () => void runAction(handleLoadAndPrepare));
  ui.reloadVoicesButton.addEventListener("click", () => void runAction(handleReloadVoices));
  ui.saveVoiceButton.addEventListener("click", () => void runAction(handleSaveUploadedVoice));
  ui.sampleSelect.addEventListener("change", fillTextFromSample);
  ui.voiceSelect.addEventListener("change", syncControls);
  ui.textInput.addEventListener("input", syncControls);
  ui.modelDirPathInput.addEventListener("change", syncControls);
  ui.sampleModeSelect.addEventListener("change", () => void runAction(saveTtsSettings));
  ui.enableWeTextProcessingToggle.addEventListener("change", () => void runAction(saveTtsSettings));
  ui.enableNormalizeTtsTextToggle.addEventListener("change", () => void runAction(saveTtsSettings));
  ui.initialPlaybackDelayInput.addEventListener("change", () => void runAction(saveTtsSettings));
  ui.threadCountInput.addEventListener("change", () => {
    state.prepared = false;
    syncControls();
    void runAction(saveTtsSettings);
  });
  ui.attnImplementationSelect.addEventListener("change", () => void runAction(saveTtsSettings));
  ui.voiceCloneMaxTextTokensInput.addEventListener("change", () => void runAction(saveTtsSettings));
  ui.ttsMaxBatchSizeInput.addEventListener("change", () => void runAction(saveTtsSettings));
  ui.codecMaxBatchSizeInput.addEventListener("change", () => void runAction(saveTtsSettings));
  ui.synthesizeButton.addEventListener("click", () => void runAction(() => runSynthesis(false)));
  ui.streamingButton.addEventListener("click", () => void runAction(() => runSynthesis(true)));
  ui.stopButton.addEventListener("click", () => void runAction(stopPlayback));
  renderSelectedModelPath();
  try {
    if (isDefaultPackagedBrowserModelPath(initialPath) && !await packagedBrowserModelExists(initialPath)) {
      syncControls();
    ...
    fillTextFromSample();
    setStatus("Runtime metadata loaded. Click Load And Prepare to warm up.");
  } catch (error) {
    logLine(`Initial metadata load skipped: ${error.message || String(error)}`);
    setStatus("Enter a valid model path, then click Load And Prepare.");
  }
}
